...
import SelfTester from "../tester/integration-tester";
  ...
  private async integrationDetection(): Promise<void> {
    ...
    this.overlayStateRef.current.setCurrentTestRunData(() => {
      ...
      if (testResult.currentPageType === PageType.LANDING) {
        testResult.landingPageResult.integrationTester =
          sovSelfTester.getTestResultData();
      } else {
        testResult.successPageResult.integrationTester =
          sovSelfTester.getTestResultData();
      }
      return testResult;
    });
